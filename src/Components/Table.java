...
public class Table extends JPanel {
    ...
    public void search(String keyword) {
        currentSearch = keyword != null ? keyword.trim() : "";

        Object[][] dataRows;

        if (currentSearch.isEmpty()) {
            dataRows = CrudManager.read("SELECT * FROM " + entity);
        } else {
            String query = buildSearchQuery(currentSearch);

            Object[] params = new Object[formStructure.length];
            Arrays.fill(params, "%" + currentSearch.toLowerCase() + "%");

            dataRows = CrudManager.read(query, params);
        }
    ...
    private String buildSearchQuery(String keyword) {
        StringBuilder query = new StringBuilder("SELECT * FROM ").append(entity).append(" WHERE ");
        for (int i = 0; i < formStructure.length; i++) {
            String columnName = ((String) formStructure[i][0]).toLowerCase();
            query.append("LOWER(COALESCE(CAST(`").append(columnName).append("` AS CHAR), '')) LIKE ?");
            if (i < formStructure.length - 1) {
                query.append(" OR ");
            }
        }
    ...
    private class ActionCellRenderer extends JPanel implements TableCellRenderer {
        ...
        public ActionCellRenderer() {
            ...
            cm.resizer("/Images/pen.png", updateLabel);
            cm.resizer("/Images/bin.png", deleteLabel);
        }
        ...
        public ActionCellEditor() {
            ...
            updateLabel.addMouseListener(new MouseAdapter() {
                ...
                public void mouseClicked(MouseEvent e) {
                    System.out.println("Update clicked for row: " + currentRow);
                    fireEditingStopped();
                    if (currentRow >= 0) {
                        Object[] rowData = new Object[dataTable.getColumnCount() - 1];
                        for (int i = 0; i < dataTable.getColumnCount() - 1; i++) {
                            rowData[i] = dataTable.getValueAt(currentRow, i);
                        }
            ...
            deleteLabel.addMouseListener(new MouseAdapter() {
                @Override
                public void mouseClicked(MouseEvent e) {
                    System.out.println("Delete clicked for row: " + currentRow);
                    fireEditingStopped();
                    if (currentRow >= 0) {
    ...
    private void openUpdateForm(Object[] rowData) {
        DynamicForm updateForm = new DynamicForm(formStructure, entity, displayEntity, rowData, this);
